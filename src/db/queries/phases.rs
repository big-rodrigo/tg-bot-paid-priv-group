...
pub async fn create(
    ...
    let (id,): (i64,) = db_query_as!(pool, (i64,),
        "INSERT INTO phases (name, description, position, phase_type) VALUES (?, ?, ?, ?) RETURNING id",
        [name, description, position, phase_type], fetch_one)?;
    Ok(id)
}
...
pub async fn update(
    ...
    db_execute!(pool,
        ...
        [name, description, position, active, phase_type, id])?;
    Ok(())
}
